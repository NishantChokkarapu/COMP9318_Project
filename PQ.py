...
def kcluster(rows,k,centers):# Cited from Programming Collective Intelligence
...
def pq(input_data, p, centroids, iterations):
    ...
    # Using K-means L1 norm assign each point of a subvector into a cluster based on
    # intitial centroids given to us

    final_cluster = kcluster(sub_vector_list[0],256,centroids)
    return final_cluster
    # print(sub_vector_list[0][1])

    # print(sub_vector_list[0].shape)
    # print(centroids.shape)
    # print("\n")
    # print(centroids)

start = time.time()
clust = pq(Data_File, 2, Centroids_File, 20)
end = time.time()
